...
export default function EventPage() {
  ...
  useEffect(() => {
    localStorage.setItem('potluck_auto_claim', String(autoClaim));
  }, [autoClaim]);
  ...
  async function loadAll(evSlug?: string) {
  ...
  // Aggregations
  const { totals, mine, contribs } = useMemo(() => {
  ...
  async function increment(itemId: string) {
    ...
    const { error } = await supabase
      .from('claims')
      .insert({ item_id: itemId, claimer_name: myName.trim(), qty: 1 });
    if (error) {
      alert('Hata: ' + error.message);
      return;
    }
    ...
    <div className='space-y-4'>
      ...
      <div className='flex items-center justify-between gap-3'>
        ...
        <div className='shrink-0 flex items-center gap-2'>
          <p>Adın: </p>
          <span className='rounded-full bg-gray-900 text-white px-3 py-1 text-sm'>
            {myName}
          </span>
          <button
            ...
            {items.map((it, idx) => {
              ...
              return (
                <tr key={it.id} className='border-t'>
                  ...
                  <td className='p-2 text-gray-700'>
                    {creators || <span className='text-gray-400'>—</span>}
                  </td>
                </tr>
              );
            })}
