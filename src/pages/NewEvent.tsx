...
export default function NewEvent() {
	...
	const onSubmit = async (e: FormEvent) => {
		e.preventDefault();
		if (!title || !slug || !hostName) {
			alert('Lütfen etkinlik başlığı ve adınızı girin.');
			return;
		}

		const { data: ev, error } = await supabase
			.from('events')
			.insert({ title, slug, host_name: hostName, created_by_name: hostName })
			.select()
			.single();
		...
		<div className='space-y-4'>
			...
			<form className='space-y-3' onSubmit={onSubmit}>
				...
				<p className='text-sm text-gray-500'>
					Link: <span className='font-mono'>/e/{slug || 'slug'}</span>
				</p>
				<button type='submit' className='w-full rounded-xl bg-gray-900 text-white py-2 font-semibold'>
					Oluştur
				</button>
			</form>
		</div>
